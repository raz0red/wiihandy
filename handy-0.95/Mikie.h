...
#ifdef WII
        // Have the period adjust based on the current frame rate
        int samplePeriod = (HANDY_SYSTEM_FREQ*((float)wii_get_max_frames()/(float)60))/HANDY_AUDIO_SAMPLE_FREQ;
#else
        int samplePeriod = HANDY_AUDIO_SAMPLE_PERIOD;
#endif

				for(;gAudioLastUpdateCycle+samplePeriod<gSystemCycleCount;gAudioLastUpdateCycle+=samplePeriod)
				{
					// Output audio sample
          ...
          // Wii
          ULONG prev = gAudioBufferPointer;
					gAudioBufferPointer%=HANDY_AUDIO_BUFFER_SIZE;
          if( prev > gAudioBufferPointer )
          {
            gAudioSkipCount++;
          }
				}
